...
if(Yii::$app->user->identity->role!=1){
	$depts[] = Yii::$app->utility->get_dept($dept_id);
	$model->manager_dept=$dept_id;
}else{
	$dept_id=NULL;
	$depts = Yii::$app->utility->get_dept($dept_id);
}
...
<?php if(isset($_GET['view']) || ($this->context->action->id=='addnewproject' && !empty($model->project_id))) {?>
<script>
...
<div class="row">
 <input type="hidden" class="form-control form-control-sm" id="project_id" name="ProjectList[key_encript]" value="<?=Yii::$app->utility->encryptString($model->project_id);?>" readonly="">
    <div class="col-sm-4"><?= $form->field($model, 'project_name')->textInput(['class'=>'form-control form-control-sm', 'maxlength' => true, 'placeholder'=>$label['project_name'], 'title'=>$label['project_name'], 'autocomplete'=>'off' ]) ?></div>
    ...
    <div class="col-sm-12"><?= $form->field($model, 'description')->textInput(['rows' => 2, 'class'=>'form-control form-control-sm', 'title'=>$label['description'], 'placeholder'=>$label['description'] ]) ?></div>
    
	 <div class="col-sm-4"><?= $form->field($model, 'manager_dept')->dropDownList($depts, ['prompt' => 'Select Department', 'class'=>'js-example-basic-multiple form-control form-control-sm', 'title'=>$label['manager_dept']]) ?></div>
    <div class="col-sm-4">
	<?=$form->field($model,'contact_person')->dropDownList(['' => '--Select--'],['class'=>'js-example-basic-multiple form-control form-control-sm'])->label('Employee')?>
	 </div>
    ...
    <div class="col-sm-12">
	<?= $form->field($model, 'address')->textInput(['rows' => 2, 'class'=>'form-control form-control-sm', 'title'=>$label['address'], 'placeholder'=>$label['address'] ])->label($label['address'].' of funding agencies') ?>
	</div>
    <?php 
    if(!empty($model->project_id)){?>
...
<?php if($this->context->action->id=='addnewproject' && !empty($model->project_id)) {?>
	<div class="col-sm-12 form-control">
	 ...
	 <label class="control-label" for="project-Cost_Breakdown" style="color: #000;">Project Cost Breakdown (Optional) </label>
