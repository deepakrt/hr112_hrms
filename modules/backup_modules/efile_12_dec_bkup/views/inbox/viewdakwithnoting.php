...
<div class='row'>
     ...
     */
    $show = '';
    // if(($fileinfo['file_category_id'] == '2' AND Yii::$app->user->identity->e_id == '200017') OR ($fileinfo['file_category_id'] == '1' AND Yii::$app->user->identity->e_id == '200043') OR (Yii::$app->user->identity->role == '2')){ $show='Y'; }
    if((Yii::$app->user->identity->e_id == '200017') OR (Yii::$app->user->identity->e_id == '200043') OR (Yii::$app->user->identity->role == '2')){ $show='Y'; }

    if($show == 'Y'){
        $fid = Yii::$app->utility->encryptString($fileinfo['file_id']);
        $closedUrl = Yii::$app->homeUrl."efile/inbox/closefile?securekey=$menuid&key=$fid";
        echo "<div class='col-sm-6'><a href='$closedUrl' id='closefile' class='btn btn-danger btn-sm'>Click here to close file</a></div>";
    }
    if($movement['fwd_to'] == 'CC'){
        ...
        echo "<div class='col-sm-6 text-right'><a href='$ccUrl' id='ccmovefile' class='btn btn-danger btn-sm'>Click here to Move File in CC Files and will hide from Inbox</a></div>";
    }
//        echo "<pre>";print_r($fileinfo['file_category_id']);
    ?>
</div>
<?php if(!empty($filedocs) OR !empty($notes)){ 
	if(!empty($filedocs) AND !empty($notes)){ ?>
		<div class='text-center'>
			<button type='button' class='btn btn-dark btn-xs' id='full_btn_note' onclick='fulscreenDoc("N")'>View Fullscreen Note</button>
			<button type='button' class='btn btn-success btn-xs' id='full_btn_default' onclick='fulscreenDoc("R")'>Default</button>
			<button type='button' class='btn btn-dark btn-xs' id='full_btn_document' onclick='fulscreenDoc("D")'>View Fullscreen Document</button>
		</div>
	<?php }
?>
