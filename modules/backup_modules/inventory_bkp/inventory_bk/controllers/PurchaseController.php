...
class PurchaseController extends Controller
{
    ...
    public function actionUpdate_status_discuss(){
		$role = Yii::$app->user->identity->role;
  		...
  		$status	=1;
		if($req_id!='' && $role==7){
			Yii::$app->inventory->update_status_discuss($req_id,$status);
			echo 1;
		}else{echo 0;}
	}
	...
	public function actionGetmsjs(){
		$role = Yii::$app->user->identity->role;
  		$req_id	=$_POST['req_id'];$html='';
  		if($req_id!=''){
			$connection=   Yii::$app->db;
			$connection->open();
			$sql="SELECT * FROM `store_material_purchase_request_msj` WHERE `req_id` = :param_req_id";
			$command = $connection->createCommand($sql); 
			...
			if($result){
			 $html.="<tr>";
			 $html.="<td> Prev. Discussion</td>";
			 $html.="</tr>";
			foreach($result as $k=>$res){ 
				 
			 $html.="<tr>";
			 $html.="<td> ".($k+1).') '.$res['msj']."</td>";
			 $html.="</tr>";
		   }
			}
			if($role==3){
	 ...
	 public function actionInsert_msj(){
		$role = Yii::$app->user->identity->role;
  		$req_id	=$_POST['req_id'];
  		$msj	=$_POST['msj']; 
 		if($req_id!='' && $msj!='' && $role==3){
			$connection=   Yii::$app->db;
			$connection->open();
			$sql="INSERT INTO `store_material_purchase_request_msj` (`req_id`, `msj`) VALUES ($req_id, '$msj')";
			$command = $connection->createCommand($sql); 
			$result=$command->execute();
 			$connection->close();
			Yii::$app->inventory->update_status_discuss($req_id,2);
			echo 'Message Sent..';
