...
?>
		...
		<?=$form->field($model,'cost_centre',['options' => ['class'=>'form-group col-sm-5']])->dropDownList($cost_centre,['prompt' => '--Select--','disabled'=>true])?>
		 
		<?php $category = ArrayHelper::map($category, 'ITEM_CAT_CODE', 'ITEM_CAT_NAME'); ?>
		<?=$form->field($model,'category',['options' => ['class'=>'form-group col-sm-5']])->dropDownList($category,['prompt' => '--Select--'])?>
		...
		<?=$form->field($model,'item',['options' => ['class'=>'form-group col-sm-5']])->dropDownList(['' => '--Select--'])->label('Item')?>
