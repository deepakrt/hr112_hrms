    ...
    <table id="dataTableShow1" class="display" style="width:100%">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td align="center" 
				<?php if($c['flag']>7  && $role!=21){ ?> 
				style="cursor:pointer;" data-toggle="modal" data-target="#myModal3"
				<?php } ?>>
				<p id="<?=$c['id']?>" <?php if($c['flag']>7 && $role!=21){ ?>  class="model_<?=$c['id']?> getdetails footer-content"<?php } ?>>
				...
				<?php 
					...
					echo $statusmsj = rtrim($statusmsj,'Y');
					 ...
					 if($status==0){
						echo $c['Status'];
    	    	      }  if(@$c['discuss_WCH']==1 && ($role=='3' || $role=='2')){ ?><hr style='margin: 7px 7px -12px 9px;'><br>
				...
				&nbsp;&nbsp;&nbsp;&nbsp;<a target="_blank" href="download?securekey=<?=$menuid?>&id=<?=base64_encode($c['id'])?>">
					<img src="<?=Yii::$app->homeUrl?>images/pdf.png" title="Download"  style="cursor:pointer;width: 25px;">  </a><br>
