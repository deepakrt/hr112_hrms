            ...
            <tr id="row_<?=$c['item_id']?>">
		     ...
		     <div class='row' style="margin: 10px;">
			  ...
			  <select class="form-control col-sm-4" name="purchase_empid" id="purchase_empid">
			   ...
			   <select required id="ipurchase_mod" class="form-control col-sm-3" name="ipurchase_mod">
			 ...
			 <input class="form-control" placeholder="Rejection Remarks" style="display:none;" type="text" id="reject_remarks" value="">		<span id='reason_gem' style='display:none;width: 100%;'>  
				<b>Specify reason for not purchasing the above item through Gem/CPP: </b>  
 				<input placeholder="Specify reason for not purchasing the above item through Gem/CPP" style="margin-top: 8px;" type="text" id="remarks" class="form-control" name="remarks" value=''/>
				<br></span> 
	...
	$('#ipurchase_mod').on('change', function () {
	 ...
	 $(".apr_rej").click(function(){
            ...
            var v_nos= ids.join(",");
             ...
             var ipurchase_mod= $("#ipurchase_mod").val();
			 if(ipurchase_mod==''){
 				swal({
					  title: "Please Select Purchase Mode",
					  text: '',icon: "warning",
  					});
				return false;
				}
			...
			swal({
				...
				if (isConfirm) {
					 $.ajax({
							...
							type:'POST',
							data:{v_nos:v_nos,purchase_empid:purchase_empid,ipurchase_mod:ipurchase_mod, remarks:remarks,_csrf:csrftoken},
							datatype:'json',
							success:function(data){
								if(data){
								swal({
