...
class PurchaseController extends Controller
{
	...
	public function actionGetmsjs(){
		...
		$html='';
  		if($req_id!=''){
			...
			if($result){ $html.="<th colspan='2'> Prev. Discussion</th>";}
			 ...
			 if($role==2){
				 $html.="<tr>";
				 $html.='<th colspan="2"> <div class="col-sm-12 text-center1" style="margin: 7px 0 0 107px;">
       With: <button type="button" class="btn btn-sm '.$typeclass.'" id="initiate_file" onclick="change_type(1)">Director</button>
        ...
        <input type="hidden" id="type" name="type" value="'.$type.'" readonly=""><input type="hidden" id="req_id" name="req_id"  value="'.$req_id.'" readonly=""></div><hr></th>';
				 $html.="</tr>";
			 }
			if($result){
			...
			foreach($result as $k=>$res){ 
				 ...
				 $html.="<td width='25%'> ".($k+1).") ".$res['role'].": </td>";
				 $html.="<td> ".$res['msj']."</td>";
				 $html.="</tr>";
		    }
			}
			if($role==3 || $role==2){
			 $html.="<tr>";
			 $html.="<td colspan='2'><br><textarea rows='3' cols='30' placeholder='Enter Message' id='dis_msj'></textarea><input type='button' id='$req_id' value='Send' style='margin: -22px 0 0 8px;' class='insert_msj btn btn-primary'></td>";
			 $html.="</tr>";}$html.="</table>";
			echo $html;
		}else{echo 0;}
	}
