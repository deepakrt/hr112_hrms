...
$tabledata.="<td><a href='javascript:' class='removeitem' id='".$c['item_id']."'>X</a></td>";
	...
	$tabledata.="<td><b>".$c['ipurchase_status'].$mod."</b></td>";
...
<form action="" id="reqform" method="post" onsubmit="return false;" >
	 ...
	 <div class="form-group col-sm-12" style="width:100%;">
		...
		<?php if($maindata['flag']=='9' || $maindata['flag']=='12' || $maindata['flag']=='11' || $maindata['flag']=='8'){ ?>
					<td align="left"><b>Not Available (StoreInc)</b><br><?=date('d-m-Y',strtotime($maindata['storeinc_action_date']));?></td>
				<?php } ?>
		</tr>
		...
		<tr>
			...
			<td align="left"> <b> Funds Available (Finance Manager) </b><br><?=date('d-m-Y',strtotime($maindata['FM_action_date']))?></td>
			<?php } ?>
		...
		<?php  } $pcost='0';if(Yii::$app->user->identity->role==6){  ?>
            <tr>
				<td colspan="2"><b>Funds are available in Project </b>
				<span>
					<select name="project_id" id="project_id" class="form-control">
						<option value=''>--Select--</option>
					<?php $projects=Yii::$app->projects->pr_get_projects();
						foreach($projects as $proj){
							$sel='';if($maindata['project_id']==$proj['project_id']){
								$pcost=$proj['project_cost'];
								$sel='selected="selected"';
							}
							echo "<option $sel data-pcost='".$proj['project_cost']."' value='".$proj['project_id']."'>".$proj['project_name']."</option>";
						}
			 ?>
					</select>	
					</span></td>
		 ...
		 <tr>
				<td colspan="2"><b>Funds are available in <?=$maindata['project']?> </b>
				 </td>
 			 </tr>
<?php }    ?>
			<tr class="project_cost">
				<td> <span class="tcost">Project Total Cost:<b> <?=$pcost?></b> </span> <button <?php if($pcost==0){echo "style='display:none'";} ?> class="get_cost_details" onclick="get_cost_details()">Cost Details</button></td>
				 
					 </tr>
		<tr><td colspan="2" id="cost_details"></td></tr>
		<?php  //} ?>
			<tr>
				<td colspan="2">
				<div class='row'>
		...
		<div class="form-group col-sm-6">
		 ...
		 <div class="form-group col-sm-12" style="text-align: center;">
			<label>
				...
				<?php if($maindata['discuss_WCH']!=1){ ?>
					<button type="button" <?php if($maindata['discuss_WCH']==1){ echo "disabled";} ?> class="discuss btn btn-info">Discuss <?php //echo $role.$maindata['approved_by_CH']?></button>     
				...
				<?php }else{ ?>
				<span style="color:red;" class="getmsjs">Already Sent to (HOD/Indentor) for Discussion. </span>
...
var role='<?=$role?>';
var approved_by_CH='<?=$maindata['approved_by_CH']?>';
		 	...
		 	var csrftoken=$("#_csrf").val();
 			 $.ajax({
					...
					success:function(data){
						if(role==7 && approved_by_CH==''){
						 $(data).insertBefore('.discuss');
						}else{
	 ...
	 $('.reject').on('click', function () {
		...
		swal({
 		...
 		$('#project_id').on('change', function () {
			var pcost= $('option:selected', "#project_id").attr('data-pcost');
			$('.project_cost').show();
			$("#cost_details").html(''); 
			$("#project_head").val(''); 
			$("#project_funds").val(''); 
			if(pcost!=''){$('.get_cost_details').show();}else{$('.get_cost_details').hide();}
			 $('.project_cost .tcost').html("Project Total Cost:<b> "+pcost);
		});
			...
			swal({
				...
				if (isConfirm) {
					  $.ajax({
							url:BASE_URL+'inventory/purchase/update_pur_req_heads?securekey='+securekey,
							type:'POST',
							data:{project_funds:project_funds, project_head:project_head, req_id:req_id, project:project, project_id:project_id, _csrf:csrftoken},
							datatype:'json',
							success:function(data){
								swal("Updated Successfully! ", "", "success")
								.then((value) => {
		...
		$('.discuss').on('click', function () { 
			var req_id=$("#req_id").val();
			var CH_remarks=$("#CH_remarks").val();
			 var csrftoken=$("#_csrf").val();
  				  $.ajax({
					url:BASE_URL+'inventory/purchase/update_status_discuss?securekey='+securekey,
					type:'POST',
					data:{req_id:req_id,remarks:CH_remarks,_csrf:csrftoken},
					datatype:'json',
					success:function(data){
						  setTimeout(function(){ window.location.reload();  }, 1500);
						swal("Notification Sent Successfully! ", "", "success")
						.then((value) => {
							$('.discuss').attr('disabled',true);
							$("body .Replied").html('Discussion');
							$("body .Replied").css('color','red');
						}); 
...
function get_cost_details() {
	var project_id=$("#project_id").val();
 	var csrftoken=$("#_csrf").val();
	  $.ajax({
		url:BASE_URL+'inventory/purchase/get_project_bcost?securekey='+securekey,
		type:'POST',
		data:{project_id:project_id,_csrf:csrftoken},
		datatype:'json',
		success:function(data){
			$("#cost_details").html(data); 
		}
	});
	}
	
	
function showdata(dis,value) {
	$("#project_head").val($(dis).attr('id')); 
	$("#project_funds").val(value); 
}
