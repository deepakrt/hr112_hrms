...
if(isset($_GET['securekey']) AND !empty($_GET['securekey'])){
    ...
    $lists = Yii::$app->utility->get_grievance_details(null,null,$emp_id); 

}
?>

<div class="col-sm-12 text-right">
<a style="margin-bottom:10px;" href="<?=Yii::$app->homeUrl?>employee/grievance/create?securekey=<?=$menuid?>" class="btn btn-success btn-sm mybtn">Add New grievance</a>
</div>
   <div class="col-sm-12">
            ...
            if(!empty($lists))
            {
             ...
             //   $editUrl = Yii::$app->homeUrl."admin/manageemployees/updateemployee?securekey=$menuid&empid=$encry";
               ...
               <td><?=$Uploadedby?></td>
                <td style="padding: 0;" >
                    <?php if($l['status']==0) {?>
<a class="btn btn-outline-danger" href="<?=Yii::$app->homeUrl?>employee/grievance/view?securekey=<?=$menuid.'&id='.$appid ?>">Preview</a>&nbsp;&nbsp;<?php
                    } else { ?><a class="btn btn-outline-success" href="<?=Yii::$app->homeUrl?>employee/grievance/view?securekey=<?=$menuid.'&id='.$appid ?>">View</a><?php } ?>
