...
class ManageleavesController extends \yii\web\Controller
{
    ...
    public function actionHolidaycalendar($val=2021){
        $this->layout = '@app/views/layouts/admin_layout.php';
        $menuid = Yii::$app->utility->decryptString($_GET['securekey']);
        $menuid = Yii::$app->utility->encryptString($menuid);
        ...
        die;*/
        return $this->render('holidaycalendar', ['HolidaysList'=>$HolidaysList,'menuid'=>$menuid]);
    }
    public function actionCheckholiday() {

       /* echo '<pre>';
         print_r($_GET);
         */
         $holiday_type = Yii::$app->utility->decryptString($_GET['key']);
         $holiday_date = $_GET['dateval'];
         ...
         $result = Yii::$app->hr_utility->hr_check_holidays($year,$month,$holiday_date,$holiday_type,$name );
    ...
    public function actionCreateholidaycalendar() {
        $this->layout = '@app/views/layouts/admin_layout.php';
        $menuid = Yii::$app->utility->decryptString($_GET['securekey']);
        $menuid = Yii::$app->utility->encryptString($menuid);
        $url = Yii::$app->homeUrl."admin/manageleaves/holidaycalendar?securekey=$menuid";
        //if(isset($_POST['sdate']) AND !empty($_POST['sdate'])){
           
           //echo "<pre>";print_r($_POST); die;
        if(isset($_POST['Holidays']) AND !empty($_POST['Holidays'])){
        ...
        //echo '<pre>';
        //print_r($post);
        //die;   //echo $_POST['Holidays']['sholidaytype'].'<br>';
                    //echo $_POST['Holidays']['year'].'<br>';
        $post = $_POST['Holidays'];
        $year = Yii::$app->utility->decryptString($post['year'][0]);
        $err = 0;

            for($i=0;$i<count($post['sholidaytype']);$i++){
                date_default_timezone_set('Asia/Kolkata');
                 //$holiday_type = $post['sholidaytype'][$i];
                 $holiday_type = Yii::$app->utility->decryptString($post['sholidaytype'][$i]);
                 $holiday_date = $post['sdate'][$i];
                 $holiday_date = trim(preg_replace('/[^0-9-]/', '', $holiday_date));
                 $name = $post['sname'][$i];
                 $holiday_date = date("Y-m-d",strtotime($holiday_date));  
                 $month = date("m", strtotime($holiday_date));
                
                //echo $year.'<br>'.$month.'<br>'.$holiday_date.'<br>'.$holiday_type;
                //die;
                $result = Yii::$app->hr_utility->hr_add_update_holidays_chart($year,$month,$holiday_date,$holiday_type,$name);
               if($result != '1'){
                    $err = 1;
               }
             ...
             */
            $logs['year']=$year;
            $logs['holidaytype']=$holiday_type;
            $logs['holidaydate']=$holiday_date;
           
            $jsonlogs = json_encode($logs);
           if($err == '0'){
                Yii::$app->utility->activities_logs("Holidays Calendar", NULL, NULL, $jsonlogs, "Holidays Calendar : Details Added Successfully.");
                
                Yii::$app->getSession()->setFlash('success', 'Details Added Successfully.');
                return $this->redirect($url);
            }else{
                Yii::$app->utility->activities_logs("Master Data", NULL, NULL, $jsonlogs, "Holidays Calendar : Entry Not Added. Contact Admin.");
                Yii::$app->getSession()->setFlash('danger', 'Entry Not Added. Contact Admin.');
                return $this->redirect($url);
            }
        }
        
        $HolidaysList = Yii::$app->hr_utility->hr_get_master_leave_type(NULL);
        return $this->render('createholidaycalendar', ['menuid'=>$menuid, 'HolidaysList'=>$HolidaysList]);
    }
...
//                        echo "<pre>";print_r($leave);die;
    ...
    public function actionUpdateleaves(){
        ...
        //save
         ...
         if(isset($_POST['UpdateLeave']) AND !empty($_POST['UpdateLeave'])){ 
             if(isset($_POST['leav_id']) AND !empty($_POST['leav_id'])){

            $leaveid = Yii::$app->utility->decryptString($_POST['leav_id']);
           // $addmore = $_POST['addmore'];
            $addmore = trim(preg_replace('/[^0-9-.]/', '', $_POST['addmore'])); 
            $levInfo = Yii::$app->utility->get_addmore_leaves($leaveid);
            $total_leave= $levInfo['total_leaves'];
           $bal_leaves = $levInfo['balance_leaves'];
            $emp_code = $levInfo['employee_code'];
           $addtotal_leave=$total_leave + $addmore;
           $addtotal_balance=$bal_leaves+$addmore; 
           $res = Yii::$app->utility->add_more_leaves($leaveid,$addtotal_leave,$addtotal_balance);
           ...
           if(empty($emp_code)){
            ...
            $this->layout = '@app/views/layouts/admin_layout.php';
            Yii::$app->getSession()->setFlash('success', "Leave Assigned Successfully. "); 
          //   $r_url='updateleaves?securekey='.$menuid;
    ...
    public function actionUpdateleavetoemployee()
     {
          $menuid = Yii::$app->utility->decryptString($_GET['securekey']);
        $menuid = Yii::$app->utility->encryptString($menuid);
        $url = Yii::$app->homeUrl."admin/manageleaves/updateleaves?securekey=$menuid";     
        if(isset($_GET['securecode']) AND !empty($_GET['securecode']) AND !empty($_GET['leve'])){
               $leaveid = Yii::$app->utility->decryptString($_GET['leve']);  
            $emp_code = Yii::$app->utility->decryptString($_GET['securecode']);
              $levInfo = Yii::$app->utility->get_addmore_leaves($leaveid);
              // echo "<pre>";
              // print_r(  $levInfo);
              if($levInfo['total_leaves']!=20)
              {
                        $bal_leaves = $levInfo['balance_leaves']+12;
                        $totalleave=20;
                        $res = Yii::$app->utility->add_more_leaves($leaveid,$totalleave,$bal_leaves);
              }     
      ...
      public function actionAddmoreleaves() {
        if(isset($_GET['code']) AND !empty($_GET['code'])){
            $code = Yii::$app->utility->decryptString($_GET['code']);    
             $code_enc = $_GET['code'];       
            if(empty($code)){ 
           ...
           // $info = Yii::$app->utility->get_employee_leaves($code);
           //  $levInfo = Yii::$app->utility->get_addmore_leaves($code);
         //echo "<pre>"; print_r($levInfo); die;
            ...
            //     die;
            // }
           // $tot_leave=$levInfo['total_leaves'] ;
             $html = '<div class="row">
    <div class="col-sm-6">
        <label>Add More Leave</label>
        <input type="text" name="addmore" class="form-control form-control-sm" placeholder="Add Leave" required=""  >
        <input type="hidden" name="leav_id" id="empcode" value='.$code_enc.' />
    </div> </div><br><div class="row">
    <div class="col-sm-6">
     <label></label>
    <input type="submit" name="save" class="btn btn-success btn-sm sl" value="Save">
  </div>
    </div>
    ';
            $result['Status']='SS';
