...
//    echo "<pre>";print_r($depts); die;
    foreach($depts as $d){
        ...
        $id = base64_encode($d['dept_id']);
        
        $new[$i]['dept_id'] = $id;
        $new[$i]['dept_name'] = $d['dept_name'];
        $i++;
    }
    $depts = $new;
}
...
<input type="hidden" name="Employee[e_id]" value="<?=Yii::$app->utility->encryptString($model->e_id);?>" readonly="" />
    <div class="row">
        ...
        <div class="col-sm-3"><?= $form->field($model, 'personal_email')->textInput(['placeholder'=>'Personal Email', 'class'=>'form-control form-control-sm', 'readonly'=>true, 'maxlength' => true]) ?></div>
	...
	<div class="row">
            <div class="col-sm-12"><input type='checkbox' id="sameAddress" name="Employee[sameAddress]" />&nbsp Same as Correspondence</div>
