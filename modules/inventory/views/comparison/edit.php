...
<a style="margin-bottom:10px;" href="<?=Yii::$app->homeUrl?>inventory/comparison/index?securekey=<?=$menuid?>" class="btn btn-success btn-sm mybtn">view</a>
    ...
    <?= $form->field($model, 'Qty', ['options' => ['class'=>'form-group col-sm-4']])->textInput(['class'=>'form-control form-control-sm'])?>
                    ...
                    <?= $form->field($model, 'tax' , ['options' => ['class'=>'form-group col-sm-4']])->radioList([1 => 'Include', 0 => 'Exclude']);?>
 ...
 <div class="form-group">
       <?= Html::submitButton('Update', ['name'=>'Update','class' => 'btn btn-primary']) ?>
    </div>
    <?php ActiveForm::end(); ?>
...
<script>
	...
	$.ajax({
		...
		success:function(data){
			...
			$('#comparisonreport-item_code').val(ITEM_CODE);
			var alt= $('option:selected', "#comparisonreport-item_code").attr('alt');
 			   var units= $('option:selected', "#comparisonreport-item_code").attr('label1');
			   $("#item_alt").val(alt);
			   $("#comparisonreport-measuring_unit option:contains(" + units + ")").prop('selected', 'selected');
		}
	  });
	var item_id= $('#comparisonreport-item_cat_code').val();
 					$.ajax({
						...
						success:function(data){
							...
							var SUPPLIER_CODE='<?=$model->Supplier_Code?>';
							$('#comparisonreport-supplier_code').val(SUPPLIER_CODE);
			var alt= $('option:selected', "#comparisonreport-supplier_code").attr('alt');
						}
					  });
...
<?php if(!empty($model->ITEM_CAT_CODE)){ ?> 
