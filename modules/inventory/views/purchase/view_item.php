        ...
        <tbody>
            ...
            <tr>
				...
				<?php if($data['ipurchase_status']=='Order-Declined' || $data['ipurchase_status']=='default'){ ?>
					 <select required id="ipurchase_mode" class="form-control" name="ipurchase_mod">
				 <option value="">--Select--</option>
 				 <option <?php if($data['ipurchase_mod']=='Gem'){echo "selected";} ?> value="Gem">Gem</option>
 				 <option <?php if($data['ipurchase_mod']=='CPP'){echo "selected";} ?> value="CPP">CPP</option>
 				 <option <?php if($data['ipurchase_mod']=='Local'){echo "selected";} ?> value="Local">Local</option>
 				 <option <?php if($data['ipurchase_mod']=='Online'){echo "selected";} ?> value="Online">Online</option>
 				 <option <?php if($data['ipurchase_mod']=='Email'){echo "selected";} ?> value="Email">Email</option>
            </select>
					<input type="hidden" id="oldpurchase_mod" value='<?=$data['ipurchase_mod'];?>'/>
			<?php  }else{ ?>
			...
			<?php	echo "<span style='margin: 9px 0 0 30px;'>".$data['ipurchase_mod']."</span>";} ?>
				...
				<b>Specify reason for not purchasing the above item through Gem/CPP: </b>  
 				<input style="margin-top: 8px;" type="text" <?php if($data['ipurchase_remarks']!=NULL){ ?> disabled  <?php } ?> id="remarkss" class="form-control" name="remarkss" value='<?=$data['ipurchase_remarks']?>'/>
				</td>
...
$(function(){
  		 $('#ipurchase_mode').on('change', function () {
 			swal({
				  title: "Are you sure?",
				  text: "",
				  icon: "warning",
				  ...
				  dangerMode: false,
					...
					if (!isConfirm) {
						$('#ipurchase_mode').val($('#oldpurchase_mod').val());return false;
					}else{
 						if($('#ipurchase_mode').val()!='Gem' && $('#ipurchase_mode').val()!='CPP'){
							$("#remarksrow").show();
							$("#remarkss").val('');
							$("#remarkss").attr('disabled',false);
						}else{
 							$("#remarksrow").hide();
							$("#remarkss").val('');$("#remarkss").attr('disabled',true);
						}
					}
				});
  	 	});
	 ...
	 $('.reject').on('click', function () {   
 		 ...
 		 $('.approve').on('click', function () {      
			...
			if($('#ipurchase_mode').val()==''){
					swal("Please select purchase mode!", "", "warning"); 
					return false;
				}
			// alert($('#remarkss').val());
			if(typeof $('#ipurchase_mode').val()!=='undefined'){
				if($('#ipurchase_mode').val()!='Gem' && $('#ipurchase_mode').val()!='CPP' && $('#remarkss').val()==''){
