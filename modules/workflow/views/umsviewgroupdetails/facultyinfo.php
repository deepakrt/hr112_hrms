...
<tr>
    ...
    <tbody class="list">
        ...
        foreach($facultyinfo as $facultyinfoK=>$facultyinfoV)
        {
         ...
         $class = "viewstudentgrouplists";
         $faculty_id = $facultyinfoV['faculty_id'];
         $grp_id = $facultyinfoV['grp_id'];
         $subjectId = $facultyinfoV['subjectId'];
         if($status=="Assign") $status="Unassign";
         else $status= "Assigned";
         
         $viewGroup_btn = "<input id='viewdetail$i'class='btn btn-primary $class' type='button' value='View Students'>";
