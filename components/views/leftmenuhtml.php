...
<div class="leftside" style='<?=$leftst?>'>
    ...
    <div class="col-sm-12 text-center ">
        ...
        <p style="min-width: 113%;margin-left: -10px;"><spam class='hindishow'><?=Yii::$app->user->identity->name_hindi?></spam><?=" / ". Yii::$app->user->identity->fullname;?></p>
    <p><spam class='hindishow'><?=Yii::$app->user->identity->desg_name_hindi?></spam><?=" / ". Yii::$app->user->identity->desg_name?></p>
            ...
            <select class="form-control form-control-sm text-center switchrole" name="role_id"  style="width: 67%;margin: 5px 0 5px 42px;">
                ...
                if($counts > 1){
                    foreach($userroles as $role){
                        ...
                        if($role_id != Yii::$app->user->identity->role){
                            echo "<option data='$desc' $selected value='$roleid'>$role</option>";
                        }
                    }
                }
...
//                if(!empty($dash['display_type'])){
//                    $leftmenuid = $leftMenu['menuid'];
//                    $leftUrl = $leftMenu['menu_url'];
//                    $leftmenuid = Yii::$app->utility->encryptString($leftmenuid);
//                    $url = Yii::$app->homeUrl.$leftUrl."?securekey=$leftmenuid&securekeyl=$menuid";
