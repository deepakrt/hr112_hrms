...
public function issue_str_item($PARAMID,$PARAMrole) {
	...
	public function pending_purchase_items($forward_to=NULL){
		$connection=   Yii::$app->db;
        $connection->open();
        $sql =" CALL `store_pending_purchase_items`()";
		$command = $connection->createCommand($sql); 
	...
	public function purchase_items_forward($itemids,$forward_to=NULL){
		$connection=   Yii::$app->db;
        $connection->open();
 		$eid=Yii::$app->user->identity->e_id;
		$sql =" CALL `store_pending_purchase_items_forward`(:item_ids,:forward_to)";
		$command = $connection->createCommand($sql); 
	...
	public function update_pur_req_FM_CH($flag,$req_id,$p_heads=NULL,$p_funds=NULL){
			...
			$sql="UPDATE `store_material_purchase_request` SET flag=$flag, FM_action_date=now(), approved_by_FM='$e_id', project_head='".$p_heads."' , project_funds='".$p_funds."' $reject_remarks where id=".$req_id;
